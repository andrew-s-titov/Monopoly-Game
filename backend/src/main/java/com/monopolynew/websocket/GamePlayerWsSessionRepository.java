...
public class GamePlayerWsSessionRepository {
    ...
    private final Map<UUID, Map<UUID, WebSocketSession>> activeGamePlayerSessions = new ConcurrentHashMap<>();

    public void addUserSession(@NonNull UUID gameId, @NonNull UUID userId, @NonNull WebSocketSession session) {
        activeGamePlayerSessions.computeIfAbsent(gameId, id -> new ConcurrentHashMap<>());
